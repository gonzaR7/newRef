...
object sin_mov_ni_costo extends SparkSessionWrapper  {
  ...
  def CalcularDataFrame(df_movimientos: DataFrame, df_costo_unificado_con_anterior: DataFrame, df_articulos: DataFrame, df_stock: DataFrame, fecha_inicial: String, fecha_final: String): DataFrame = {
      ...
      val df_articulos_no_movimiento_no_costo = df_fechas.join(df_articulos.select(col("codigo"), col("barras").as("codigo_barra")), Seq.empty[String], "cross")
        ...
        .join(df_costo_unificado_con_anterior.select(col("barras").as("codigo_barra"), col("fecha_vigencia_desde").as("fecha")), Seq("codigo_barra", "fecha"), "left_anti")
      ...
      val df_pre_final = df_join_stock.withColumn("movimientos_agrupados", lit(0)).select(col("codigo"), col("barras"), col("fecha_stock"), col("movimientos_agrupados"), col("existencia").as("total_unidades"), col("costo_unitario"), col("costo_anterior"), col("precio_actual_mayorista"), col("precio_actual_minorista"), col("resultado_por_tenencia"))
