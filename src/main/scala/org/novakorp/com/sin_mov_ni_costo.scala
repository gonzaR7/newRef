...
object sin_mov_ni_costo extends SparkSessionWrapper  {
  ...
  def CalcularDataFrame(df_movimientos: DataFrame, df_costo_unificado_con_anterior: DataFrame, df_articulos: DataFrame, df_stock: DataFrame, fecha_inicial: String, fecha_final: String): DataFrame = {
      ...
      val df_articulos_no_movimiento_no_costo = df_fechas.join(df_articulos.select(col("codigo").as("codigo_articulo"), col("barras").as("codigo_barra")), Seq.empty[String], "cross").join(df_movimientos, Seq("codigo_articulo", "fecha"), "left_anti").join(df_costo_unificado_con_anterior.select(col("barras").as("codigo_barra"), col("fecha_vigencia_desde").as("fecha")), Seq("codigo_barra", "fecha"), "left_anti").withColumnRenamed("codigo_articulo","codigo")
