...
object con_movimientos extends SparkSessionWrapper  {
  ...
  def CalcularDataFrame(df_con_mov_con_cu:DataFrame, df_movimientos: DataFrame , df_costo_unificado_con_anterior: DataFrame, df_stock: DataFrame,df_articulos: DataFrame,fecha_inicial: String, fecha_final: String): DataFrame = {
    ...
    val df_mov_agrupado: DataFrame = df_enganchado.groupBy("codigo_articulo", "fecha", "barras_enganchado").agg(sum(col("cantidad_movimiento") * col("multiplicador_stock").cast("int")).as("movimientos_agrupados")).withColumnRenamed("barras_enganchado","codigo_barra")
